...
const TechStack = () => {
  ...
  return (
    <section id="stack" className="py-24 border-t border-outline-variant relative overflow-hidden">
      ...
      <div className="px-margin-mobile md:px-margin-desktop max-w-container-max mx-auto relative z-10">
        <div className="text-center mb-16">
          ...
          <p className="mt-4 text-secondary max-w-2xl mx-auto">Bahasa pemrograman, framework, dan alat pendukung yang saya gunakan untuk membangun aplikasi yang modern dan berkualitas.</p>
        </div>

        <div className="space-y-12">
          {categories.map((category, catIdx) => (
            <div key={catIdx} className="reveal" style={{ transitionDelay: `${catIdx * 0.1}s` }}>
              <h3 className="text-lg font-semibold text-primary mb-6 text-center md:text-left flex items-center gap-4">
              ...
              <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-10 gap-3 md:gap-4">
                {category.skills.map((s, i) => (
                  <div 
                    key={i} 
                    className="group flex flex-col items-center justify-center gap-3 py-5 px-2 bg-surface-container-low hover:bg-surface-container rounded-2xl border border-outline-variant shadow-sm hover:shadow-lg transition-all duration-300 hover:-translate-y-1 hover:border-primary backdrop-blur-sm"
                  >
                    <div 
                      className={`text-3xl md:text-4xl transition-transform duration-300 group-hover:scale-110 drop-shadow-sm ${!s.color ? 'text-primary' : ''}`}
                      style={s.color ? { color: s.color } : {}}
                    >
                      {s.icon}
                    </div>
                    <span className="text-[10px] md:text-xs font-medium text-secondary group-hover:text-primary transition-colors">{s.name}</span>
                  </div>
                ))}
              </div>
            </div>
